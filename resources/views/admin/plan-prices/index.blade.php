...
<div class="main-section">
    ...
    .row-img { height: 40px; width: 40px; object-fit: cover; border-radius: 6px; border: 1px solid var(--border); }
    .badge-plan { display: inline-block; padding: 3px 10px; border-radius: 999px; font-size: 11.5px; font-weight: 600; background: rgba(48,61,137,.1); color: var(--accent); text-transform: capitalize; }
    ...
    <div class="app-content content container-fluid">
        <div class="cat-page">
            ...
            <div class="cat-page-header">
                <h1>Price Plans</h1>
                <a href="{{ route('admin.plan-prices.create') }}" class="btn-primary-dash">
                    ...
                    <tbody>
                        @forelse($prices as $price)
                            <tr>
                                <td>
                                    @if($price->image)
                                        <img src="{{ asset('price/'.$price->image) }}" class="row-img">
                                    @else
                                        —
                                    @endif
                                </td>
                                ...
                                <td style="text-transform: capitalize">{{ $price->montly_yearly }}</td>
                                <td><span class="badge-plan">{{ $price->plan }}</span></td>
                                <td>
                                    <div class="row-actions">
                                        <a href="{{ route('admin.plan-prices.edit', $price) }}" class="icon-btn" title="Edit">
                                            <i class="fa fa-pencil"></i>
                                        </a>
                                        <form action="{{ route('admin.plan-prices.destroy', $price) }}" method="POST"
                                            onsubmit="return confirm('Delete this price plan?');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="empty-state">No price plans added yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>

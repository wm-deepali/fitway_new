{{-- resources/views/admin/page-quote-requests/index.blade.php --}}
...
<div class="main-section">
    ...
    .badge.read { background: #e4f5e9; color: #1e7a3f; }
    ...
    <div class="app-content content container-fluid">
        <div class="cat-page">
            ...
            <div class="cat-card">
                <form action="{{ route('admin.pageQuoteRequests.index') }}" method="GET" class="filters-bar">
                    ...
                    <input type="text" name="search" class="form-control-styled" placeholder="Search name, email, mobile, page…" value="{{ request('search') }}">
                    ...
                    @if(request()->hasAny(['status', 'search']))
                        <a href="{{ route('admin.pageQuoteRequests.index') }}" class="clear-link">Clear filters</a>
                    @endif
                </form>

                @if($pageQuoteRequests->isEmpty())
                ...
                @else
                    <table class="products-table">
                        ...
                        <tbody>
                            @foreach($pageQuoteRequests as $pageQuoteRequest)
                                <tr id="page-quote-row-{{ $pageQuoteRequest->id }}">
                                    <td>
                                        <div class="cust-name">{{ $pageQuoteRequest->full_name }}</div>
                                        <div class="cust-sub">#{{ $pageQuoteRequest->id }}</div>
                                    </td>
                                    <td>{{ $pageQuoteRequest->mobile_number }}</td>
                                    <td>{{ $pageQuoteRequest->email }}</td>
                                    <td>{{ $pageQuoteRequest->page_id }}</td>
                                    <td>
                                        <span class="badge {{ $pageQuoteRequest->is_read ? 'read' : 'new' }}">
                                            {{ $pageQuoteRequest->is_read ? 'Read' : 'New' }}
                                        </span>
                                    </td>
                                    <td>{{ $pageQuoteRequest->created_at->format('d M Y, h:i A') }}</td>
                                    <td>
                    ...
                    <div class="pagination-wrap">
                        {{ $pageQuoteRequests->links('pagination::bootstrap-4') }}
                    </div>
                @endif
            </div>
...
<script>
function deletePageQuoteRequest(id) {
    Swal.fire({
        ...
        fetch(`/admin/page-quote-requests/${id}`, {
            method: 'DELETE',
            headers: {
            ...
            .then(res => {
                if (res.success) {
                    document.getElementById(`page-quote-row-${id}`).remove();

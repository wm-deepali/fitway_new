...
<div class="main-section">
    ...
    .cat-img { width: 44px; height: 44px; border-radius: 50%; object-fit: cover; border: 1px solid var(--border); }
    .cat-img-placeholder { width: 44px; height: 44px; border-radius: 50%; background: var(--bg); display: flex; align-items: center; justify-content: center; color: var(--text-hint); font-size: 16px; border: 1px solid var(--border); }
    ...
    .desc-cell { max-width: 280px; color: var(--text-secondary); font-size: 12.5px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        ...
        <div class="cat-page">
            ...
            <div class="cat-card">
                    ...
                    <form method="GET">
                        <div class="form-row-inner">
                            ...
                            <div class="filter-actions">
                                ...
                                <a href="{{ route('admin.testimonials.index') }}" class="btn-secondary-dash">
                ...
                <div class="cat-table-wrap">
                    ...
                    @php
                        function testiSortUrl($column) {
                            $direction = request('sort_by') == $column && request('sort_order_dir') == 'asc' ? 'desc' : 'asc';
                            return request()->fullUrlWithQuery(['sort_by' => $column, 'sort_order_dir' => $direction]);
                        }
                        function testiSortIcon($column) {
                            if (request('sort_by') != $column) return '<i class="fa fa-sort"></i>';
                            return request('sort_order_dir') == 'asc'
                    ...
                    <table class="cat-table">
                        <thead>
                            <tr>
                                <th>
                                    <a href="{{ testiSortUrl('id') }}" class="sort-link">ID {!! testiSortIcon('id') !!}</a>
                                </th>
                                <th>Photo</th>
                                <th>
                                    <a href="{{ testiSortUrl('name') }}" class="sort-link">Name {!! testiSortIcon('name') !!}</a>
                                </th>
                                <th>
                                    <a href="{{ testiSortUrl('company') }}" class="sort-link">Company {!! testiSortIcon('company') !!}</a>
                                </th>
                                <th>Description</th>
                                <th>
                                    <a href="{{ testiSortUrl('status') }}" class="sort-link">Status {!! testiSortIcon('status') !!}</a>
                                </th>
                        ...
                        <tbody>
                            @forelse($testimonials as $item)
                                <tr id="row{{ $item->id }}">
                                    ...
                                    <td style="color:var(--text-secondary);font-size:13px">{{ $item->company ?: '—' }}</td>
                                    ...
                                    <td>
                                        <div style="display:flex;gap:6px">
                                            <a href="{{ route('admin.testimonials.edit', ['testimonial' => $item->id, 'redirect' => request()->fullUrl()]) }}"
                                                class="action-btn" title="Edit">
                                                <i class="fa fa-pencil"></i>
                                            </a>
                                            <button class="action-btn action-btn-danger"
                                                onclick="deleteTestimonial({{ $item->id }})" title="Delete">
                                ...
                                <tr>
                                    <td colspan="7">
                                        <div class="empty-state">
                                            ...
                                            <strong style="font-size:14px;color:var(--text-primary)">No testimonials found</strong>
                                            <p>Try adjusting your search or add a new testimonial to get started.</p>
                                            <a href="{{ route('admin.testimonials.create') }}" class="btn-primary-dash">
                                                <i class="fa fa-plus"></i> Add Testimonial
                                            </a>
                                        </div>

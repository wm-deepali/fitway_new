            ...
            <div class="cat-card">
                ...
                <div class="cat-table-wrap">
                    ...
                    <table class="cat-table">
                        <thead>
                            <tr>
                                <th><a href="{{ gymSortUrl('id') }}" class="sort-link">ID {!! gymSortIcon('id') !!}</a>
                                </th>
                                <th><a href="{{ gymSortUrl('full_name') }}" class="sort-link">Name
                                        {!! gymSortIcon('full_name') !!}</a></th>
                                <th>Email</th>
                                <th>Mobile</th>
                                <th>Requirements</th>
                                <th><a href="{{ gymSortUrl('created_at') }}" class="sort-link">Received
                                        {!! gymSortIcon('created_at') !!}</a></th>
                                    ...
                                    <td>
                                        <div class="name-cell">
                                            <strong>{{ $item->full_name }}
                                                {!! !$item->is_read ? '<span class="pill pill-yes">New</span>' : '' !!}</strong>
                                        </div>
                                    </td>
                                    <td style="color:var(--text-secondary);font-size:13px">{{ $item->email }}</td>
                                    <td style="color:var(--text-secondary);font-size:13px">{{ $item->mobile_number ?: '—' }}
                                    </td>
                                    <td style="color:var(--text-secondary);font-size:13px">
                                        {{ !empty($item->requirements) ? implode(', ', $item->requirements) : '—' }}</td>
                                    <td style="color:var(--text-secondary);font-size:13px">
                                ...
                                <tr>
                                    <td colspan="7">
                                        <div class="empty-state">
                                            ...
                                            <strong style="font-size:14px;color:var(--text-primary)">No requests
                                                yet</strong>
...
<script>
    ...
    $.ajax({
        ...
        success: function (res) {
            const d = res.data;
            const reqs = d.requirements.length ? d.requirements.join(', ') : '—';
            ...
            Swal.fire({
                ...
                html: `
                    <div style="text-align:left;font-size:13px;line-height:1.7">
                        ...
                        <p><strong>Mobile:</strong> ${d.mobile_number ?? '—'}</p>
                        <p><strong>Requirements:</strong> ${reqs}</p>
                        <p><strong>Details:</strong> ${d.details ?? '—'}</p>
                        <p><strong>Received:</strong> ${d.created_at}</p>
                    </div>
                `,
                confirmButtonText: 'Close'
            });

            // Row was marked read server-side — drop the "New" pill without a full reload
            $("#row" + id + " .pill-yes").remove();
        },
        error: function () {
            Swal.fire('Error!', 'Something went wrong', 'error');
        }

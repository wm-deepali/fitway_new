{{-- resources/views/admin/minisubcategory/edit.blade.php --}}
    ...
    <div class="app-content content container-fluid">
        <div class="cat-page">
            ...
            <div class="cat-page-header">
                ...
                <a href="{{ request('redirect', route('admin.minisubcategories.index')) }}" class="btn-secondary-dash">
                    <i class="fa fa-arrow-left"></i> Back to List
                </a>
            </div>

            <div class="cat-card">
                <form action="{{ route('admin.minisubcategories.update', $minisubcategory->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <select id="category_id" name="category_id"
                            ...
                            @foreach($parentCategories as $parent)
                                <option value="{{ $parent->id }}"
                                    {{ old('category_id', $minisubcategory->category_id) == $parent->id ? 'selected' : '' }}>
                    ...
                    <div class="form-field">
                        ...
                        <select id="sub_cat_id" name="sub_cat_id"
                            ...
                            @foreach($subCategories as $sub)
                                <option value="{{ $sub->id }}"
                                    {{ old('sub_cat_id', $minisubcategory->sub_cat_id) == $sub->id ? 'selected' : '' }}>
                                    {{ $sub->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-field">
                        <label for="name">Mini Sub Category Name</label>
                        <input type="text" id="name" name="name"
                            class="form-control-styled @error('name') is-invalid @enderror"
                            value="{{ old('name', $minisubcategory->name) }}" required>
                    ...
                    <div class="form-field">
                        ...
                        @if($minisubcategory->image)
                            <img src="{{ asset('storage/' . $minisubcategory->image) }}" class="current-img-preview" alt="{{ $minisubcategory->name }}">
                        @endif
                    ...
                    <div class="form-field toggle-row">
                        <label class="switch">
                            <input type="checkbox" name="status" value="1" {{ old('status', $minisubcategory->status) ? 'checked' : '' }}>
                    ...
                    <div class="form-field">
                        ...
                        <input type="text" id="meta_title" name="meta_title" class="form-control-styled"
                            value="{{ old('meta_title', $minisubcategory->meta_title) }}">
                    </div>

                    <div class="form-field">
                        <label for="meta_keywords">Meta Keywords</label>
                        <input type="text" id="meta_keywords" name="meta_keywords" class="form-control-styled"
                            value="{{ old('meta_keywords', $minisubcategory->meta_keywords) }}">
                    </div>

                    <div class="form-field">
                        <label for="meta_description">Meta Description</label>
                        <textarea id="meta_description" name="meta_description" rows="4"
                            class="form-control-styled">{{ old('meta_description', $minisubcategory->meta_description) }}</textarea>
                    </div>

                    <div class="form-actions">
                        <button type="submit" class="btn-primary-dash">
                            <i class="fa fa-check"></i> Update Mini Sub Category
                        </button>
                        <a href="{{ request('redirect', route('admin.minisubcategories.index')) }}" class="btn-secondary-dash">Cancel</a>
                    </div>

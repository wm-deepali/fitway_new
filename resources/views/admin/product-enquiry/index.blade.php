...
<div class="main-section">
    ...
    .addr-cell { max-width: 220px; color: var(--text-secondary); font-size: 12.5px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
            ...
            <div class="cat-card">
                        ...
                        <div class="form-row-inner">
                            <div class="filter-group">
                                ...
                                <input type="text" name="search" value="{{ request('search') }}"
                                    class="filter-control" placeholder="Search name, email, city…">
                            </div>
                ...
                <div class="cat-table-wrap">
                    ...
                    <table class="cat-table">
                        <thead>
                            <tr>
                                <th><a href="{{ peSortUrl('id') }}" class="sort-link">Sr. No. {!! peSortIcon('id') !!}</a></th>
                                <th><a href="{{ peSortUrl('name') }}" class="sort-link">Name {!! peSortIcon('name') !!}</a></th>
                                <th>Email</th>
                                <th>Mobile</th>
                                <th><a href="{{ peSortUrl('city') }}" class="sort-link">City {!! peSortIcon('city') !!}</a></th>
                                <th><a href="{{ peSortUrl('state') }}" class="sort-link">State {!! peSortIcon('state') !!}</a></th>
                                <th>Address</th>
                                <th>Pin</th>
                        ...
                        <tbody>
                            @forelse($enquiries as $index => $item)
                                <tr id="row{{ $item->id }}">
                                    ...
                                    <td style="color:var(--text-secondary);font-size:13px">{{ $item->city ?: '—' }}</td>
                                    <td style="color:var(--text-secondary);font-size:13px">{{ $item->state ?: '—' }}</td>
                                    <td class="addr-cell" title="{{ $item->address }}">{{ $item->address ?: '—' }}</td>
                                    <td style="color:var(--text-secondary);font-size:13px">{{ $item->pin ?: '—' }}</td>
                                    <td>
                                ...
                                <tr>
                                    <td colspan="9">
                                        <div class="empty-state">
                                            ...
                                            <strong style="font-size:14px;color:var(--text-primary)">No product enquiries yet</strong>

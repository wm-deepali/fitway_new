            ...
            <div class="cat-card">
                        ...
                        <div class="form-row-inner">
                            <div class="filter-group">
                                ...
                                <input type="text" name="search" value="{{ request('search') }}"
                                    class="filter-control" placeholder="Search name, email, service…">
                            </div>
                            <div class="filter-actions">
                                <button type="submit" class="btn-primary-dash">
                                    <i class="fa fa-search"></i> Search
                                </button>
                                <a href="{{ route('admin.inquiries.index') }}" class="btn-secondary-dash">
                ...
                <div class="cat-table-wrap">
                    ...
                    <table class="cat-table">
                        <thead>
                            <tr>
                                <th><a href="{{ inqSortUrl('id') }}" class="sort-link">ID {!! inqSortIcon('id') !!}</a></th>
                                <th><a href="{{ inqSortUrl('name') }}" class="sort-link">Name {!! inqSortIcon('name') !!}</a></th>
                                <th><a href="{{ inqSortUrl('service') }}" class="sort-link">Service {!! inqSortIcon('service') !!}</a></th>
                                <th>Message</th>
                                <th><a href="{{ inqSortUrl('created_at') }}" class="sort-link">Received {!! inqSortIcon('created_at') !!}</a></th>
                                ...
                                <tr id="row{{ $item->id }}">
                                    ...
                                    <td>
                                        <div class="name-cell">
                                            ...
                                            <small>{{ $item->email }} @if($item->mobile_number) &middot; {{ $item->mobile_number }} @endif</small>
                                        </div>
                                    </td>
                                    <td style="color:var(--text-secondary);font-size:13px">{{ $item->service ?: '—' }}</td>
                                ...
                                <tr>
                                    <td colspan="6">
                                        <div class="empty-state">
                                            <div class="empty-icon"><i class="fa fa-circle-question"></i></div>
                                            <strong style="font-size:14px;color:var(--text-primary)">No inquiries yet</strong>
                                            <p>Inquiry form submissions will show up here.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
    ...
    fetch("{{ url('admin/inquiries') }}/" + id)
        ...
        .then(res => {
            ...
            document.getElementById('detailModalBody').innerHTML = `
                ...
                <div class="detail-row"><div class="detail-label">Mobile</div><div class="detail-value">${i.mobile_number ?? '—'}</div></div>
                <div class="detail-row"><div class="detail-label">Service</div><div class="detail-value">${i.service ?? '—'}</div></div>
                <div class="detail-row"><div class="detail-label">Message</div><div class="detail-value">${i.message ?? '—'}</div></div>
            `;

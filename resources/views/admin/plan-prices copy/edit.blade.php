...
<div class="main-section">
    ...
    .current-img { height: 70px; width: 70px; object-fit: cover; border-radius: var(--radius-sm); border: 1px solid var(--border); margin-top: 8px; }
    </style>
    ...
    <div class="app-content content container-fluid">
        <div class="cat-page">
            ...
            <div class="cat-card">
                <form action="{{ route('admin.plan-prices.update', $price) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        @for($i = 1; $i <= 4; $i++)
                            <div class="form-field">
                                ...
                                <input type="text" id="title_{{ $i }}" name="title_{{ $i }}"
                                    class="form-control-styled" value="{{ old('title_'.$i, $price->{'title_'.$i}) }}"
                    ...
                    <div class="form-row">
                        <div class="form-field">
                            ...
                            <input type="number" id="days" name="days" min="0"
                                class="form-control-styled @error('days') is-invalid @enderror"
                                value="{{ old('days', $price->days) }}">
                            @error('days') <div class="form-error">{{ $message }}</div> @enderror
                        </div>
                        <div class="form-field">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-field">
                            ...
                            <select id="montly_yearly" name="montly_yearly"
                                ...
                                <option value="month" {{ old('montly_yearly', $price->montly_yearly) == 'month' ? 'selected' : '' }}>Month</option>
                                <option value="year" {{ old('montly_yearly', $price->montly_yearly) == 'year' ? 'selected' : '' }}>Year</option>
                            </select>
                    ...
                    <div class="form-field">
                        ...
                        <select id="plan" name="plan"
                            ...
                            <option value="medium" {{ old('plan', $price->plan) == 'medium' ? 'selected' : '' }}>Medium</option>
                            <option value="standard" {{ old('plan', $price->plan) == 'standard' ? 'selected' : '' }}>Standard</option>
                            <option value="premium" {{ old('plan', $price->plan) == 'premium' ? 'selected' : '' }}>Premium</option>
                        </select>
                    ...
                    <div class="form-field">
                        ...
                        @if($price->image)
                            <div>
                                <img src="{{ asset('price/'.$price->image) }}" class="current-img">
                            </div>
                        @endif
                    </div>
